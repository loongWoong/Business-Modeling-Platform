...
import { domainAPI, modelAPI } from '../../services/api'
...
const DomainMap = () => {
  ...
  useEffect(() => {
    const loadData = async () => {
      try {
        ...
        // 后端返回的是数组，需要转换为包含domains和edges的格式
        const domains = Array.isArray(domainData) ? domainData : (domainData.domains || [])
        const domainEdges = domainData.edges || []
        
        // 获取模型数据
        try {
          const modelData = await modelAPI.getAll()
          // 处理模型数据和边数据
          const allModels = Array.isArray(modelData) ? modelData : (modelData.models || [])
          const modelEdges = modelData.edges || []
          
          setAllData({
            domains: domains,
            models: allModels,
            edges: [...domainEdges, ...modelEdges]
          })
          setDomains(domains)
  ...
  const handleCreateDomain = async () => {
    try {
      const domain = await domainAPI.create(newDomain)
      setDomains([...domains, domain])
      setIsModalOpen(false)
      setNewDomain({ name: '', description: '', owner: '' })
      // 重新加载数据以获取更新后的列表
      const domainData = await domainAPI.getAll()
      const updatedDomains = Array.isArray(domainData) ? domainData : (domainData.domains || [])
      setDomains(updatedDomains)
      setAllData(prev => ({ ...prev, domains: updatedDomains }))
    } catch (error) {
  ...
  return (
    <div className="domain-map">
      ...
      <div style={{ padding: '12px 16px', backgroundColor: '#f0f2f5', borderBottom: '1px solid #e0e0e0' }}>
        <span style={{ fontWeight: 'bold' }}>业务域地图</span>
