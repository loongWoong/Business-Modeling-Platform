...
import { modelAPI, domainAPI, sharedAttributeAPI, relationAPI, modelTableAssociationAPI, datasourceAPI, indicatorAPI, dataAPI } from '../../services/api';
...
const ModelDetail = () => {
  ...
  useEffect(() => {
    const loadData = async () => {
      ...
      try {
        // 使用新API获取模型详情（包含properties和relations）
        const modelData = await modelAPI.getById(modelId);
        const foundModel = modelData.model;
        ...
        if (foundModel) {
          ...
          try {
            ...
            const domain = domains.find(d => d.id === foundModel.domainId);
            const updatedModel = {
              id: parseInt(modelId),
            ...
            // 获取当前业务域的共享属性
            try {
              const sharedAttrData = await sharedAttributeAPI.getAll(updatedModel.domainId);
              setSharedAttributes(Array.isArray(sharedAttrData) ? sharedAttrData : []);
            } catch (error) {
              console.error('Failed to fetch shared attributes:', error);
            }
          } catch (error) {
        ...
        try {
          const allRelationData = await relationAPI.getAll();
        ...
        // 获取模型-数据源表关联数据
        try {
          const associationData = await modelTableAssociationAPI.getAll(modelId);
        ...
        // 获取模型绑定的指标
        try {
          const indicatorData = await indicatorAPI.getByModelId(modelId);
          const indicatorsArray = Array.isArray(indicatorData) ? indicatorData : [];
          setBoundIndicators(indicatorsArray);
        } catch (error) {
          console.error('Failed to fetch bound indicators:', error);
        }

        // 获取所有可用指标
        try {
          const indicatorData = await indicatorAPI.getAll();
          const indicatorsArray = Array.isArray(indicatorData) ? indicatorData : [];
        ...
        try {
          ...
          const data = await dataAPI.getAll(modelId);
      ...
      <div style={{ padding: '12px 16px', backgroundColor: '#f0f2f5', borderBottom: '1px solid #e0e0e0' }}>
        ...
        {model?.domainId && (
          <>
            ...
            >
              {model?.domainName || (currentDomain ? currentDomain.name : '...')}
            </span>
          </>
        )}
        <span style={{ marginRight: '8px' }}>&gt;</span>
        <span style={{ fontWeight: 'bold' }}>{model?.name || '加载中...'}</span>
